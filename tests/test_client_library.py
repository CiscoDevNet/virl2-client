...
def test_import_lab_from_path_virl(mocked_session, tmp_path: Path):
    ...
    with patch.object(Lab, "sync", autospec=True) as sync_mock:
        lab = client_library.import_lab_from_path(
            topology=(tmp_path / "topology.virl").as_posix())
...
def test_auth_and_reauth_token():
    ...
    responses.add(
        responses.GET, 'https://0.0.0.0/fake_url/api/v0/wait_for_lld_connected')
    responses.add(
        responses.GET, 'https://0.0.0.0/fake_url/api/v0/wait_for_lld_connected', status=401)
    responses.add(
        responses.GET, 'https://0.0.0.0/fake_url/api/v0/wait_for_lld_connected')
    responses.add(
        responses.GET, 'https://0.0.0.0/fake_url/api/v0/wait_for_lld_connected', status=401)
...
def test_client_library_init_allow_http():
    client_library = ClientLibrary(
        "http://somehost", "virl2", "virl2", allow_http=True)
...
def test_client_library_init_url(monkeypatch, via, parms):
    ...
    if fail:
        with pytest.raises((InitializationError, requests.exceptions.InvalidURL)) as exc:
            client_library = ClientLibrary(
                url=url, username="virl2", password="virl2")
    else:
...
def test_client_library_init_user(monkeypatch, via, parms):
    ...
    if fail:
        with pytest.raises((InitializationError, requests.exceptions.InvalidURL)) as exc:
            client_library = ClientLibrary(
                url=url, username=user, password="virl2")
    else:
...
def test_client_library_init_password(monkeypatch, via, parms):
    ...
    if fail:
        with pytest.raises((InitializationError, requests.exceptions.InvalidURL)) as exc:
            client_library = ClientLibrary(
                url=url, username="virl2", password=password)
    else:
        client_library = ClientLibrary(
            url, username="virl2", password=password)
        assert client_library.username == "virl2"
...
def test_client_library_str_and_repr():
    ...
    assert repr(
        client_library) == "ClientLibrary('somehost', 'virl2', 'virl2', True, False, False)"
