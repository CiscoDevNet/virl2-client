...
# TODO: enum for lab/link/node PUT actions
# TODO: better method naming?
# TODO: error/partial handling
...
class BulkManagement:
    ...
    _URL_PARAM_MAP = {True: "true", False: "false", None: "null"}
    ...
    def __init__(
        ...
        # needed only for lab creation and import
        self._client = client
    ...
    def get_labs(
        self, lab_ids: list[str] | None = None, with_data=True
    ) -> list[dict[str, Any]]:
        """
        Get details (or IDs) of the specified labs (get all by default).
        ...
        :param with_data: Whether to get just lab IDs or full lab objects.
        :returns: A list of dictionaries with lab properties.
        """
        url = f"{self._url_for('labs')}?data={self._URL_PARAM_MAP[with_data]}"
        if lab_ids:
    ...
    def remove_labs(self, lab_ids: list[str]) -> None:
        ...
        for lab_id in lab_ids:
            if lab_id in self._client._labs:
                lab = self._client._labs[lab_id]
                self._client._remove_lab_local(lab)

    def put_labs(self, lab_ids: list[str], action: str) -> None:
        """
        Update the state of the specified labs.

        Warning: All data must be valid (support for partial results is not available
            yet).

        :param lab_ids: A list of lab IDs.
        :param action: The action to perform (start, stop, wipe).
        :returns: None.
        """
        url = f"{self._url_for('labs')}/{action}"
        self._session.put(url, json=lab_ids)
    ...
    def get_links(
        self, link_ids: list[str] | None = None, with_data=True
    ) -> list[dict[str, Any]]:
        """
        ...
        :param with_data: Whether to get just link IDs or full link objects.
        :returns: A list of dictionaries with link properties.
        """
        url = f"{self._url_for('links')}?data={self._URL_PARAM_MAP[with_data]}"
        if link_ids:
            # https://github.com/encode/httpx/discussions/1587
    ...
    def put_links(self, link_ids: list[str], action: str) -> None:
        """
        Update the state of the specified links.
        ...
        :returns: None.
        """
        url = f"{self._url_for('links')}/{action}"
        self._session.put(url, json=link_ids)
    ...
    def get_nodes(
        ...
        :param with_data: Whether to get just node IDs or full node objects.
        ...
        """
        url = (
            f"{self._url_for('nodes')}?data={self._URL_PARAM_MAP[with_data]}"
            f"&operational={self._URL_PARAM_MAP[operational]}"
            f"&exclude_configurations={self._URL_PARAM_MAP[exclude_configurations]}"
        )
        if node_ids:
            # https://github.com/encode/httpx/discussions/1587
    ...
    def post_nodes(
        ...
        """
        url = (
            f"{self._url_for('nodes')}"
            f"?populate_interfaces={self._URL_PARAM_MAP[populate_interfaces]}"
        )
    ...
    def put_nodes(self, link_ids: list[str], action: str) -> None:
        """
        Update the state of the specified nodes.

        Warning: All data must be valid (support for partial results is not available
            yet).

        :param link_ids: A list of node IDs.
        :param action: The action to perform (start, stop, wipe).
        :returns: None.
        """
        url = f"{self._url_for('nodes')}/{action}"
        self._session.put(url, json=link_ids)
    ...
    def _handle_node_response(self, response: list[dict[str, Any]]) -> list[Node]:
        ...
        for data in response:
            ...
            try:
                ...
                lab = self._client.join_existing_lab(lab_id)
            node_id = data.pop("id")
            try:
