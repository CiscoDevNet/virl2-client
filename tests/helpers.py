...
# Tests that do not assert on the user manager itself can reference
# this constant.  Tests that need to observe calls should pass their own
# Mock() to make_lab(user_management=...).
...
def make_lab(
    ...
    user_management: Mock | None = None,
) -> Lab:
    ...
    :param user_management: User management mock; uses the
        module-level USER_MANAGEMENT when None.
    ...
    return Lab(
        ...
        user_management=(
            user_management if user_management is not None else USER_MANAGEMENT
        ),
    )
