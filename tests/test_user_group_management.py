...
def test_sync_users() -> None:
    """sync_users fetches user list and stores by id."""
    session = MagicMock()
    mgr = UserManagement(session, auto_sync=False)
    session.get.return_value.json.return_value = [
        {"id": "u1", "username": "alice"},
        {"id": "u2", "username": "bob"},
    ]
    mgr.sync_users()
    assert mgr._users_by_id["u1"]["username"] == "alice"
    assert mgr._users_by_id["u2"]["username"] == "bob"
    assert mgr._last_sync_users_time > 0


def test_sync_users_if_outdated_triggers() -> None:
    """sync_users_if_outdated calls sync when interval elapsed."""
    session = MagicMock()
    mgr = UserManagement(session, auto_sync=True, auto_sync_interval=0)
    session.get.return_value.json.return_value = [{"id": "u1", "username": "a"}]
    mgr.sync_users_if_outdated()
    assert "u1" in mgr._users_by_id


def test_sync_users_if_outdated_skips() -> None:
    """sync_users_if_outdated skips sync when auto_sync is off."""
    session = MagicMock()
    mgr = UserManagement(session, auto_sync=False)
    mgr.sync_users_if_outdated()
...
def test_get_username_found() -> None:
    """get_username returns username when user exists."""
    session = MagicMock()
    mgr = UserManagement(session, auto_sync=True, auto_sync_interval=0)
    session.get.return_value.json.return_value = [
        {"id": "u1", "username": "alice"},
    ]
    assert mgr.get_username("u1") == "alice"


def test_get_username_not_found() -> None:
    """get_username returns None when user id is not in list."""
    session = MagicMock()
    mgr = UserManagement(session, auto_sync=True, auto_sync_interval=0)
    session.get.return_value.json.return_value = [
        {"id": "u1", "username": "alice"},
    ]
    assert mgr.get_username("nonexistent") is None


def test_get_username_reuses_synced_data() -> None:
    """get_username does not re-fetch within auto_sync_interval."""
    session = MagicMock()
    mgr = UserManagement(session, auto_sync=True, auto_sync_interval=9999)
    session.get.return_value.json.return_value = [
        {"id": "u1", "username": "alice"},
    ]
    mgr.sync_users()
